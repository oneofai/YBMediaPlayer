...
@implementation YBIJKPlayerManager
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _scalingMode = YBPlayerScalingModeAspectFit;
        _shouldAutoPlay = YES;
    }
    return self;
}
...
- (void)prepareToPlay {
    ...
    self.loadState = YBPlayerLoadStatePrepare;
...
- (void)play {
    if (!_isPreparedToPlay) {
        ...
        _isPlaying = YES;
        self.playState = YBPlayerPlayStatePlaying;
    }
}
...
- (void)pause {
    [self.player pause];
    _isPlaying = NO;
    self.playState = YBPlayerPlayStatePaused;
}
...
- (void)stop {
    ...
    self.playState = YBPlayerPlayStatePlayStopped;
}
...
- (void)timerUpdate {
    if (self.player.currentPlaybackTime > 0 && !self.isReadyToPlay) {
        ...
        self.loadState = YBPlayerLoadStatePlaythroughOK;
    }
...
- (void)moviePlayBackFinish:(NSNotification *)notification {
    ...
    switch (reason) {
        case IJKMPMovieFinishReasonPlaybackEnded: {
            YBPlayerLog(@"playbackStateDidChange: 播放完毕: %d\n", reason);
            self.playState = YBPlayerPlayStatePlayStopped;
        ...
        case IJKMPMovieFinishReasonUserExited: {
            YBPlayerLog(@"playbackStateDidChange: 用户退出播放: %d\n", reason);
        }
            break;
            
        case IJKMPMovieFinishReasonPlaybackError: {
            YBPlayerLog(@"playbackStateDidChange: 播放出现错误: %d\n", reason);
            self.playState = YBPlayerPlayStatePlayFailed;
            if (self.playerPlayFailed) self.playerPlayFailed(self, @(reason));
        }
            break;
            
        default:
            YBPlayerLog(@"playbackPlayBackDidFinish: ???: %d\n", reason);
            break;
    }
}
...
- (void)mediaIsPreparedToPlayDidChange:(NSNotification *)notification {
    YBPlayerLog(@"加载状态变成了已经缓存完成，如果设置了自动播放, 会自动播放");
    // 视频开始播放的时候开启计时器
    if (!self.timer) {
...
- (void)loadStateDidChange:(NSNotification*)notification {
    ...
    if ((loadState & IJKMPMovieLoadStatePlayable)) {
        YBPlayerLog(@"加载状态变成了缓存数据足够开始播放，但是视频并没有缓存完全");
        if (self.player.currentPlaybackTime > 0) {
            self.loadState = YBPlayerLoadStatePlayable;
        }
        ...
        YBPlayerLog(@"网速不好等因素导致了暂停");
        self.loadState = YBPlayerLoadStateStalled;
    } else {
        YBPlayerLog(@"加载状态变成了未知状态");
        self.loadState = YBPlayerLoadStateUnknown;
    }
}
...
- (void)moviePlayBackStateDidChange:(NSNotification *)notification {
    switch (self.player.playbackState) {
        case IJKMPMoviePlaybackStateStopped: {
            YBPlayerLog(@"播放器的播放状态变了，现在是停止状态 %d: stoped", (int)_player.playbackState);
        ...
        case IJKMPMoviePlaybackStatePlaying: {
            YBPlayerLog(@"播放器的播放状态变了，现在是播放状态 %d: playing", (int)_player.playbackState);
        }
            break;
            
        case IJKMPMoviePlaybackStatePaused: {
            YBPlayerLog(@"播放器的播放状态变了，现在是暂停状态 %d: paused", (int)_player.playbackState);
        }
            break;
            
        case IJKMPMoviePlaybackStateInterrupted: {
            YBPlayerLog(@"播放器的播放状态变了，现在是中断状态 %d: interrupted", (int)_player.playbackState);
        }
            break;
            
        case IJKMPMoviePlaybackStateSeekingForward: {
            YBPlayerLog(@"播放器的播放状态变了，现在是向前拖动状态:%d forward",(int)self.player.playbackState);
        }
            break;
        case IJKMPMoviePlaybackStateSeekingBackward: {
            YBPlayerLog(@"放器的播放状态变了，现在是向后拖动状态 %d: backward", (int)_player.playbackState);
        }
            break;
            
        default: {
            YBPlayerLog(@"播放器的播放状态变了，现在是未知状态 %d: unknown", (int)_player.playbackState);
        }
            break;
    }
}
...
- (void)setPlayState:(YBPlayerPlaybackState)playState {
    _playState = playState;
    if (self.playerPlayStateChanged) self.playerPlayStateChanged(self, playState);
}

- (void)setLoadState:(YBPlayerLoadState)loadState {
...
- (void)setScalingMode:(YBPlayerScalingMode)scalingMode {
    _scalingMode = scalingMode;
    switch (scalingMode) {
        case YBPlayerScalingModeNone:
            self.player.scalingMode = IJKMPMovieScalingModeNone;
            break;
        case YBPlayerScalingModeAspectFit:
            self.player.scalingMode = IJKMPMovieScalingModeAspectFit;
            break;
        case YBPlayerScalingModeAspectFill:
            self.player.scalingMode = IJKMPMovieScalingModeAspectFill;
            break;
        case YBPlayerScalingModeFill:
