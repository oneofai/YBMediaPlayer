...
@interface YBPlayerVolumeView ()

@property (nonatomic, strong) UIProgressView *progressView;
@property (nonatomic, strong) UIImageView *iconImageView;
@property (nonatomic, assign) YBPlayerVolumeType volumeType;
@property (nonatomic, strong) MPVolumeView *volumeView;

@end

@implementation YBPlayerVolumeView
...
- (void)updateProgress:(CGFloat)progress volumeType:(YBPlayerVolumeType)volumeType {
    ...
    if (volumeType == YBPlayerVolumeTypeSonic) {
        if (progress == 0) {
            playerImage = YBUIImageMake(@"YBPlayer_muted");
        } else if (progress > 0 && progress < 0.5) {
            playerImage = YBUIImageMake(@"YBPlayer_volume_low");
        } else {
            playerImage = YBUIImageMake(@"YBPlayer_volume_high");
        }
    } else if (volumeType == YBPlayerVolumeTypeumeBrightness) {
        if (progress >= 0 && progress < 0.5) {
            playerImage = YBUIImageMake(@"YBPlayer_brightness_low");
        } else {
            playerImage = YBUIImageMake(@"YBPlayer_brightness_high");
        }
    }
...
- (void)setVolumeType:(YBPlayerVolumeType)volumeType {
    _volumeType = volumeType;
    if (volumeType == YBPlayerVolumeTypeSonic) {
        self.iconImageView.image = YBUIImageMake(@"YBPlayer_volume_high");
    } else {
        self.iconImageView.image = YBUIImageMake(@"YBPlayer_brightness_high");
    }
}
...
- (MPVolumeView *)volumeView {
    if (!_volumeView) {
        ...
        _volumeView.frame = CGRectMake(-2000, -1000, 100, 100);
    }
    return _volumeView;
}
