...
#if __has_include(<YBMediaPlayer/YBPlayerLogManager.h>)
#import <YBMediaPlayer/YBPlayerLogManager.h>
#else
...
NSString *const YBDownloadNetworkSpeedNotificationKey = @"YBDownloadNetworkSpeedNotificationKey";
NSString *const YBUploadNetworkSpeedNotificationKey   = @"YBUploadNetworkSpeedNotificationKey";
NSString *const YBNetworkSpeedNotificationKey         = @"YBNetworkSpeedNotificationKey";

@interface YBNetworkSpeedMonitor () {
    // 总网速
    uint32_t _iBytes;
    uint32_t _oBytes;
    uint32_t _allFlow;
...
@implementation YBNetworkSpeedMonitor
...
- (void)checkNetworkSpeed {
    ...
    if (_iBytes != 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YBDownloadNetworkSpeedNotificationKey object:nil userInfo:@{YBNetworkSpeedNotificationKey:_downloadNetworkSpeed}];
        YBPlayerLog(@"downloadNetworkSpeed : %@",_downloadNetworkSpeed);
    }
    ...
    if (_oBytes != 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YBUploadNetworkSpeedNotificationKey object:nil userInfo:@{YBNetworkSpeedNotificationKey:_uploadNetworkSpeed}];
        YBPlayerLog(@"uploadNetworkSpeed :%@",_uploadNetworkSpeed);
    }
