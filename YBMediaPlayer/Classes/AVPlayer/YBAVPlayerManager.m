...
@interface YBPlayerPresentView : YBPlayerView
...
@implementation YBPlayerPresentView
...
@interface YBAVPlayerManager () {
    id _timeObserver;
    id _itemEndObserver;
    YBPlayerKVOController *_playerItemKVO;
}
...
@implementation YBAVPlayerManager
...
- (void)initializePlayer {
    ...
    YBPlayerPresentView *presentView = (YBPlayerPresentView *)self.view;
...
- (void)bufferingSomeSecond {
    ...
    if (self.isBuffering || self.playState == YBPlayerPlayStatePlayStopped) return;
    /// 没有网络
    if ([YBPlayerReachabilityManager sharedManager].networkReachabilityStatus == YBPlayerReachabilityStatusNotReachable) return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!self.isPlaying && self.loadState == YBPlayerLoadStateStalled) {
            self.isBuffering = NO;
            return;
        }
        [self play];
...
- (void)itemObserving {
    [_playerItemKVO safelyRemoveAllObservers];
    _playerItemKVO = [[YBPlayerKVOController alloc] initWithTarget:_playerItem];
    ...
    _timeObserver = [self.player addPeriodicTimeObserverForInterval:interval queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (self.isPlaying && self.loadState == YBPlayerLoadStateStalled) self.player.rate = self.rate;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([keyPath isEqualToString:kStatus]) {
            if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
                /// 第一次初始化
                if (self.loadState == YBPlayerLoadStatePrepare) {
...
- (UIView *)view {
    if (!_view) {
        _view = [[YBPlayerPresentView alloc] init];
    }
    return _view;
}
